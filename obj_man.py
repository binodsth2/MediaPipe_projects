...
cube_center = None
cube_size = 40
frame_count = 0  # Add this after your imports and before the loop
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        if cube_center:
            # ...existing code for drawing the circulating cube...
            # (as in previous answer)
            radius = 60
            speed = 0.05
            angle = frame_count * speed
            offset_x = int(radius * np.cos(angle))
            offset_y = int(radius * np.sin(angle))

            rvec = np.zeros((3, 1))
            tvec = np.array([
                [cube_center[0] + offset_x],
                [cube_center[1] + offset_y],
                [800]
            ], dtype=np.float32)
